...
include 'config.php';
require_once 'email.php'; // Assuming this file contains your SendGrid API key and other configurations
...
if ($cart_total == 0) {
    ...
    if (mysqli_num_rows($order_query) > 0) {
        ...
        $user_query = mysqli_query($conn, "SELECT name, email FROM `users` WHERE id = '$user_id'") or die('query failed');
        ...
        $email->addTo($user_email, $user_name); // Use the fetched email address
 
        // Replace the placeholders in the template with the actual values
        $email->addDynamicTemplateData("placed_on", $placed_on);
        $email->addDynamicTemplateData("name", $user_name);
