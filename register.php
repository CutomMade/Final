...
if (isset($_POST['submit'])) {
    ...
    $user_type = 'user'; // Set user_type to 'user' by default

    // Check if the email already exists in the users table with user_type 'user'
    $checkEmailQuery = "SELECT * FROM `users` WHERE email = '$email' AND user_type = 'user'";
    $result = mysqli_query($conn, $checkEmailQuery);

    if (mysqli_num_rows($result) > 0) {
        // If a duplicate email with user_type 'user' is found, display an error message
        $message[] = 'Email already registered as a user! Please use a different email address.';
    } else {
        ...
        if (mysqli_num_rows($result) > 0) {
            // If the email is found, check the user_type
            $row = mysqli_fetch_assoc($result);
            if ($row['user_type'] == 'admin') {
                // If the user_type is 'admin', set user_type to 'user'
                $user_type = 'user';
            } else {
                // If the user_type is 'user', display an error message
                $message[] = 'Email already registered as a user! Please use a different email address.';
            }
        }
    }
